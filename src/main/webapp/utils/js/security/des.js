/**
 * DES加解密js函数 2013年10月26日 16:28:28
 */
(function($w) {
	if(typeof $w.RSAUtils === 'undefined')
		var DESUtils = $w.DESUtils = {};
	 ...
	 *  DES加密函数
	 */
	DESUtils.encryptDES = function(message, key) {
		if(key){
			keyDES = key;
		} else {
			if(!keyDES) {
				keyDES = GSINFO.data('key');
				if(!keyDES) {
					try {
						keyDES = getCert(1).substr(0, 32);
					} catch (e) {
						//window.location.href = base + '/front/index.do?action=toLogin';
					}
				}
			}
		}
		return DESUtils.encryptDESByKey(keyDES, message);
	};
	
	/**
	 * DES解密函数
	 */
	DESUtils.decryptDES = function(message, key) {
		if(key){
			keyDES = key;
		} else {
			if(!keyDES) {
		...
		return DESUtils.decryptDESByKey(keyDES, message);
	};
	
	/**
	 *  DES加密函数
	 */
	DESUtils.encryptDESByKey = function(key, message) {
	    var ciphertext = DESUtils.stringToHex(DESUtils.des(key, message, 1, 0,""));
	    return ciphertext;
	};
	 ...
	 * DES解密函数
	 */
	DESUtils.decryptDESByKey = function(key, message) {
	    var plaintext = DESUtils.des(key, DESUtils.HexTostring(message), 0, 0,"");
	    return plaintext;
	};
	 ...
	 * encrypt 1加密，0解密
	 */
	DESUtils.des = function(key, message, encrypt, mode, iv) {
	    ...
	    var keys = DESUtils.des_createKeys(key);
...
//	        }
	...
	// 密钥生成函数
	DESUtils.des_createKeys = function(key) {
	 ...
	 */
	DESUtils.stringToHex = function(s) {
	 ...
	 */
	DESUtils.HexTostring = function(s) {
	    var r = "";
