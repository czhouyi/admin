package com.chinadaas.gsinfo.admin.action;

import org.springframework.beans.factory.annotation.Autowired;

import com.chinadaas.gsinfo.admin.service.IService;
import com.chinadaas.gsinfo.admin.service.UkeyStatService;
import com.chinadaas.gsinfo.admin.vo.UkeyStat;

/**
 * UkeyStat action
 * projectName: gsinfo30-admin<br>
 * desc: TODO<br>
 * date: 2014年7月8日 下午6:00:45<br>
 * @author 开发者真实姓名[裔传洲]
 */
public class UkeyStatAction extends BizBaseAction<UkeyStat> {
	
	@Autowired
	private UkeyStatService ukeyStatService;

	public void setUkeyStatService(UkeyStatService ukeyStatService) {
		this.ukeyStatService = ukeyStatService;
	}

	@Override
	public IService getBizService() {
		return ukeyStatService;
	}

}

